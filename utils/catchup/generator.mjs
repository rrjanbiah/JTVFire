...
import { getEpg, epochTS2yyyymmdd } from "./epg.mjs";
...
async function getUrl(id, start, end, retry = 0) {
  try {
    ...
    const epgDataRes = await getEpg(id, start);
    if (!epgDataRes.status) {
      return "";
    }
    const epgData = epgDataRes.data;
    ...
    var options = {
      ...
      body: `stream_type=Catchup&showtime=${epgData.showtime.replaceAll(
        ":",
        ""
      )}&programId=${epgData.srno}&srno=${epochTS2yyyymmdd(
        epgData.startEpoch
      )}&channel_id=${id}&begin=${start}&end=${end}`,
    };
    const catchupGetUrl =
      "https://jiotvapi.media.jio.com/playback/apis/v1/geturl?langId=6";
    let response = await fetch(catchupGetUrl, options);
    jdebug("url", catchupGetUrl, "options", options, "response", response);
...
async function getMasterM3u8(id, start, end) {
  jdebug(`getMasterM3u8(${id}, ${start}, ${end})`);
...
async function getLiveM3u8(url, vbegin, vend, cookie) {
  jdebug(`getLiveM3u8(${url}, ${vbegin}, ${vend}, ${cookie})`);
  try {
    const userDataJiotv = JSON["parse"](
      fs["readFileSync"]("tokenData.jiotv", { encoding: "utf8", flag: "r" })
    );
    ...
    jdebug("url", url, "response", response);
...
export async function getM3u8(id, start, end, m3u8, vbegin, vend) {
  jdebug(`getM3u8(${id}, ${start}, ${end}, ${m3u8}, ${vbegin}, ${vend})`);
  // jdebug("getM3u8(id, start, end, m3u8, vbegin, vend)", id, start, end, m3u8, vbegin, vend);
